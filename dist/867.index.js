...
const crossSpawn = __webpack_require__(546);
const stripFinalNewline = __webpack_require__(4075);
const npmRunPath = __webpack_require__(9440);
const onetime = __webpack_require__(4528);
const makeError = __webpack_require__(801);
const normalizeStdio = __webpack_require__(3126);
const {spawnedKill, spawnedCancel, setupTimeout, validateTimeout, setExitHandler} = __webpack_require__(9575);
const {handleInput, getSpawnedResult, makeAllStream, validateInputSync} = __webpack_require__(543);
const {mergePromise, getSpawnedPromise} = __webpack_require__(1466);
const {joinCommand, parseCommand, getEscapedCommand} = __webpack_require__(7530);
...
const {signalsByName} = __webpack_require__(6549);
...
const onExit = __webpack_require__(7470);
...
const isStream = __webpack_require__(2434);
const getStream = __webpack_require__(4379);
const mergeStream = __webpack_require__(8595);
...
const bufferStream = __webpack_require__(3037);
...
Object.defineProperty(exports, "__esModule", ({value:true}));exports.signalsByNumber=exports.signalsByName=void 0;var _os=__webpack_require__(857);

var _signals=__webpack_require__(1247);
var _realtime=__webpack_require__(5041);
...
Object.defineProperty(exports, "__esModule", ({value:true}));exports.getSignals=void 0;var _os=__webpack_require__(857);

var _core=__webpack_require__(3143);
var _realtime=__webpack_require__(5041);
...
const path = __webpack_require__(6928);
const pathKey = __webpack_require__(6689);
...
const mimicFn = __webpack_require__(9384);
...
if (!processOk(process)) {
  ...
  var signals = __webpack_require__(9291)
...
const minSatisfying = __webpack_require__(976)
...
;// CONCATENATED MODULE: ./node_modules/is-wayland/index.js
...
function isWayland() {
	...
	if (external_node_process_.env.WAYLAND_DISPLAY) {
		return true;
	}
	...
	if (external_node_process_.env.XDG_SESSION_TYPE === 'wayland') {
		return true;
	}
...
// EXTERNAL MODULE: ./node_modules/run-jxa/node_modules/execa/index.js
var node_modules_execa = __webpack_require__(2923);
...
;// CONCATENATED MODULE: ./node_modules/subsume/index.js
...
;// CONCATENATED MODULE: ./node_modules/macos-version/index.js




const isMacOS = external_node_process_.platform === 'darwin';
...
function macOSVersion() {
	...
	if (!version) {
		const file = external_node_fs_.readFileSync('/System/Library/CoreServices/SystemVersion.plist', 'utf8');
...
function isMacOSVersionGreaterThanOrEqualTo(version) {
	...
	return node_modules_semver.gte(macOSVersion(), clean(version));
}
...
function macos_version_assertMacOSVersionGreaterThanOrEqualTo(version) {
	version = version.replace('10.16', '11');
...
;// CONCATENATED MODULE: ./node_modules/run-jxa/index.js
...
async function runJxa(input, arguments_) {
	macos_version_assertMacOSVersionGreaterThanOrEqualTo('10.10');
	const {stderr} = await node_modules_execa('osascript', commandArguments, prepareOptions(input, arguments_));
...
;// CONCATENATED MODULE: ./node_modules/clipboard-image/index.js
/* globals $, ObjC */
...
async function writeClipboardImages(filePaths) {
	...
	const paths = filePaths.map(path => path instanceof URL ? (0,external_node_url_.fileURLToPath)(path) : path);
...
const shouldUseMain = (0,is_unicode_supported/* default */.A)();
...
const defaultVerboseFunction = ({
	...
	return `${(0,base/* gray */.wm)(`[${timestampString}]`)} ${(0,base/* gray */.wm)(`[${commandId}]`)} ${color(icon)} ${color(message)}`;
};
...
const COLORS = {
	command: () => base/* bold */.Cr,
	output: () => identity,
	ipc: () => identity,
	error: ({reject}) => reject ? base/* redBright */.GS : base/* yellowBright */.$z,
	duration: () => base/* gray */.wm,
};
...
const cleanupOnExit = (subprocess, {cleanup, detached}, {signal}) => {
	...
	const removeExitHandler = (0,mjs/* onExit */.cf)(() => {
		subprocess.kill();
	});
	(0,external_node_events_.addAbortListener)(signal, () => {
		removeExitHandler();
...
const clipboard = {
	async copy(options) {
		try {
			await execa_execa('termux-clipboard-set', options);
		} catch (error) {
			handler(error);
		}
	},
	async paste(options) {
		try {
			const {stdout} = await execa_execa('termux-clipboard-get', options);
			return stdout;
		} catch (error) {
			handler(error);
		}
...
const xsel = 'xsel';
const xselFallbackPath = external_node_path_.join(linux_dirname, '../fallbacks/linux/xsel');
...
const hasXselFallback = external_node_fs_.existsSync(xselFallbackPath);
...
const xselWithFallback = async (argumentList, options) => {
	try {
		const {stdout} = await execa_execa(xsel, argumentList, options);
		return stdout;
	} catch (xselError) {
		if (!hasXselFallback) {
			throw linux_makeError(xselError);
		}

		try {
			const {stdout} = await execa_execa(xselFallbackPath, argumentList, options);
			return stdout;
		} catch (fallbackError) {
...
;// CONCATENATED MODULE: ./node_modules/clipboardy/lib/wayland.js
...
const wayland_makeError = (command, error) => {
	if (error.code === 'ENOENT') {
...
const tryWaylandWithFallback = async (command, arguments_, options, fallbackMethod) => {
	try {
		const result = await execa_execa(command, arguments_, options);
		...
		throw wayland_makeError(command, error);
	}
};
...
/* harmony default export */ const wayland = (wayland_clipboard);
...
const macos_clipboard = {
	copy: async options => execa_execa('pbcopy', {...options, env}),
	async paste(options) {
		const {stdout} = await execa_execa('pbpaste', {...options, env});
		return stdout;
	},
...
const execFilePromises = (0,external_node_util_.promisify)(external_node_child_process_.execFile);
...
function systemArchitectureSync() {
	const {arch, platform, env} = external_node_process_;

	// Detect Node.js x64 binary running under Rosetta 2 on a ARM64 Mac.
	if (platform === 'darwin' && arch === 'x64') {
		const stdout = external_node_child_process_.execFileSync('sysctl', ['-inq', 'sysctl.proc_translated'], {encoding: 'utf8'});
		return stdout.trim() === '1' ? 'arm64' : 'x64';
	}
	...
	if (platform === 'linux') {
		const stdout = external_node_child_process_.execFileSync('getconf', ['LONG_BIT'], {encoding: 'utf8'});
		if (stdout.trim() === '64') {
			return 'x64';
		}
	}
...
;// CONCATENATED MODULE: ./node_modules/powershell-utils/index.js
...
const powershell_utils_execFile = (0,external_node_util_.promisify)(external_node_child_process_.execFile);

const powerShellPath = () => `${external_node_process_.env.SYSTEMROOT || external_node_process_.env.windir || String.raw`C:\Windows`}\\System32\\WindowsPowerShell\\v1.0\\powershell.exe`;
...
const encodeCommand = command => external_node_buffer_.Buffer.from(command, 'utf16le').toString('base64');
...
const executePowerShell = async (command, options = {}) => {
	...
	return powershell_utils_execFile(
...
const executePowerShellSync = (command, options = {}) => {
	const {
		powerShellPath: psPath,
		...execFileOptions
	} = options;
...
// Check if the bundled fallback binary exists on disk. It may not exist when running in a bundled environment (e.g. Node.js SEA, webpack, esbuild).
const hasWindowsBinaryFallback = external_node_fs_.existsSync(windowBinaryPath);
...
const runPowerShell = (script, options) => execa_execa(
	powerShellPath(),
...
const runBinary = (arguments_, options) => execa_execa(
	windowBinaryPath,
	arguments_,
	{
		windowsHide: true,
...
;// CONCATENATED MODULE: ./node_modules/clipboardy/lib/wsl.js
...
const wsl_clipboard = {
	async copy(options) {
		await execa_execa('clip.exe', [], options);
	},
	copySync(options) {
...
/* harmony default export */ const wsl = (wsl_clipboard);
...
const platformLib = (() => {
	switch (external_node_process_.platform) {
		...
		case 'android': {
			if (external_node_process_.env.TERMUX_VERSION === undefined) {
